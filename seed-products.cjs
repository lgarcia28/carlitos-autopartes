/**
 * Seed script: Carga 15 productos de muestra en Firestore.
 * Ejecutar con: node seed-products.cjs
 * Usa el Firebase Admin SDK con Application Default Credentials.
 */

const { initializeApp, applicationDefault, cert } = require("firebase-admin/app");
const { getFirestore } = require("firebase-admin/firestore");

// Initialize with project ID (uses gcloud credentials or service account)
...
async function seed() {
  console.log("🚀 Iniciando carga de productos de muestra...\n");
  ...
  for (const prod of seedProducts) {
    ...
    console.log(`  ✅ Preparado: ${prod.name}`);
  }

  console.log("\n📤 Enviando batch a Firestore...");
  await batch.commit();
  console.log("\n🎉 ¡Listo! 15 productos cargados exitosamente en la colección 'products'.");
}
